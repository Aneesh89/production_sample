...
import { ChangeGuard } from './ChangeGuard';


export function ChangeofGDGuard(props) {
  ...
  const [guardChange, setGuardChange] = useState([]);

  // add change of guard gd
    ...
    <>
      ...
      >
        {({ errors, touched, isSubmitting, dirty, resetForm, values, setFieldValue, setFieldTouched }) => (
          <Col  >
            <ChangeGuard data={guardChange} setGuardChange={setGuardChange} />
            <Form>
              <FormGroup row>
                <Col>
                  <Row>
